<template>
	<div class="image-upload" @click="$refs.myFiles.click()">
		<input type="file" ref="myFiles" class="custom-file-input" @input="convertImgToBase64($event)"/>
		<div class="image-upload__container" v-if="!imgPreview">
			<div class="add-icon"></div>
		</div>
		<img :src="imgPreview" class="image-upload__preview" v-else />
...
<script>
	export default {
        ...
        this.imgSrc.mimeType = file.type;
				this.imgSrc.name = 'image-' + Date.now() + '.' + imageExtension;
...
<style lang="scss">
@import "./../../style/components/_image-upload.scss";
</style>
